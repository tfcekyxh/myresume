...
4. 专业技能的拆分（重要）：原文「专业技能」模块中的每一行、每一个编号项都是独立的一条 skill，必须逐条输出成数组元素，绝不允许把整段合并成一条。
   - 即使原文是一整段连续文字、中间没有换行，也要按句末标点（。；）拆成多条：每个由「掌握 / 熟悉 / 熟练 / 精通 / 了解」等动词引出、描述一个独立技能主题的句子算一条。
   - 只有当一个句子内部确实在讲同一个技能主题时才不拆。
5. summary 与 points 的区分（重要）：
...
6. 时间、专业名词、数字保留原文写法，不要翻译或改写；去除多余空白。
7. 按原文顺序输出，保持模块内条目顺序。`
